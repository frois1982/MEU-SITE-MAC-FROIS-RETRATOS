...
export default async function handler(req: any, res: any) {
  ...
  // ── CORS — permite chamadas da página de captura ────────────
  res.setHeader("Access-Control-Allow-Origin", "https://www.macfrois.com.br");
  ...
  // O data center fica após o hífen na API key: "abc123-us21" → "us21"
  ...
  const payload = {
    ...
    status: "subscribed",                // "pending" para double opt-in
    merge_fields: {
      FNAME: (nome ?? "").trim(),
    },
  ...
  try {
    ...
    // 200 = novo inscrito | 400 com title "Member Exists" = já estava na lista
